// Computes Lunaria localization status, caching it per latest-commit-date so a
// dev/build only recomputes when history changes. Includes a shallow-repo
// fallback (CI checkouts are often shallow) that clones the full history.
// Vendored from microsoft/aspire.dev (MIT).

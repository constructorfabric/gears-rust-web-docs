// Minimal Starlight plugin that renders a Lunaria translation-status dashboard
// as part of the normal `astro build` (no separate `lunaria build` CLI step).
// Vendored from microsoft/aspire.dev (MIT) and trimmed for this repo.
//
// It injects a route (default `/i18n`) backed by src/components/lunaria/Dashboard.astro
// and exposes the resolved plugin config + shallow-repo flag to that component via a
// `virtual:lunaria-starlight` module.
...
const LunariaStarlightConfigSchema = z
  .object({
    ...
    // When true (and building), derive the Lunaria `locales`/`defaultLocale`
    // from the Starlight config. We keep it false so lunaria.config.json stays
    // the single source of truth and CI never rewrites it.
